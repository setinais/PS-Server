...
use App\Avaliacao;
use App\Charts\AvaliacoesEstatistica;
...
class AvaliacaoController extends VoyagerBaseController
{
    ...
    public function estatisticas(Request $request){
        ...
        foreach ($informacoes as $k => $informacoe){
            $labels[] = $informacoe->nome;
            $avaliacaos = $informacoe->avaliacaos;

            $soma_atendimento = 0;
            $soma_servico = 0;
            $soma_estrutura = 0;
            $qntd = count($avaliacaos);

            foreach ($avaliacaos as $ke => $avaliacao){
                $soma_atendimento += $avaliacao->tempo_de_espera;
        ...
        $chart->labels($labels);
        $chart->dataset('Atendimento', 'horizontalBar', $atendimento)->options([
            'backgroundColor' => 'rgba(106, 196, 10, 0.4)',
        ]);
        $chart->dataset('Serviço', 'horizontalBar', $servico)->options([
            'backgroundColor' => 'rgba(229, 27, 32, 0.4)',
        ]);
        $chart->dataset('Estrutura', 'horizontalBar', $estrutura)->options([
            'backgroundColor' => 'rgba(9, 58, 197, 0.4)',
        ]);

        $chart->options([
//            'title' => 'Avaliações das unidades e hospoutal',
            'tooltip' => [
                'show' => true
            ],
        ]);
