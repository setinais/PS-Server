@section('media-manager-user')
    <div>
            ...
            <div class="breadcrumb-container">
                <ol class="breadcrumb filemanager">
                    <li class="media_breadcrumb" v-on:click="setCurrentPath(-1)">
                        ...
                        <strong>{{ __('Banners de Campanhas do Aplicativo Mobile') }}</strong>
                    </li>
        ...
        <input id="qntd_banner" type="hidden" value="0"/>
        <!-- Image Modal -->
...
<script>
    Vue.component('media-manager-user', {
        template: `@yield('media-manager-user')`,
        props: {
            basePath: {
                type: String,
                default: '/'
        ...
        methods: {
            getFiles: function() {
                var vm = this;
                vm.is_loading = true;
                $.post('{{ route('media.user.files') }}', { folder: vm.current_folder, _token: '{{ csrf_token() }}', details: vm.details }, function(data) {
                    console.log(data);
                    vm.files = [];
            ...
            fileIs: function(file, type) {
                if (typeof file === 'string') {
                    if (type == 'image') {
                        return this.endsWithAny(['jpg', 'png'], file);
                    }
            ...
            crop: function(mode) {
                ...
                $.post('{{ route('media.user.crop') }}', postData, function(data) {
                    if (data.success) {
